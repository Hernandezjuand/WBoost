...
// PDFLib imports removed as we now use direct HTML/CSS rendering
...
const logStep = (step, details) => {
  console.log(`[Gemini] Step ${step}:`, details);
};
...
export const extractJobDetails = async (jobDescription, apiKey) => {
  logStep('JobDetails', 'Starting job details extraction');
  ...
  if (typeof jobDescription === 'object' && jobDescription !== null) {
    if (jobDescription.description) {
      ...
      try {
        ...
        logStep('JobDetails', 'Warning: Could not stringify job description object');
      }
    }
  }
  ...
  if (!processedJobDescription || typeof processedJobDescription !== 'string' || processedJobDescription.trim() === '') {
    logStep('JobDetails', 'Error: Empty or invalid job description', {
  ...
  logStep('JobDetails', 'Processed job description', {
  ...
  const messages = [
    {
      role: 'system',
      content: 'You are an AI assistant specialized in analyzing job descriptions. Extract key details from the job description and return a JSON object. PRIORITIZE the core role requirements - these are critical. If a detail is not found, use empty strings. Be thorough in extracting ALL required qualifications and skills.'
    },
    {
      role: 'user',
      content: `Job Description: ${processedJobDescription}

Extract the following details in a JSON object:
1. company: Company name
2. role: Job title/role (VERY IMPORTANT - extract this precisely)
3. location: Work location
4. salary: Salary range if mentioned
5. keySkills: A comma-separated list of the MOST important technical or role-specific skills required
6. keyRequirement: The single most critical qualification/experience needed
7. experience: Years of experience required
8. education: Education requirements
9. requirements: Full paragraph describing core requirements
10. responsibilities: Main job duties
11. seniority: Entry/Junior/Mid/Senior level
12. industry: Primary industry

Be precise and comprehensive. Return ONLY the raw JSON object, no markdown formatting or additional text.`
    }
  ];

  try {
    logStep('JobDetails', 'Sending extraction request');
    const response = await makeRequest(messages, apiKey);
    
    logStep('JobDetails', 'Cleaning response');
    const cleanedResponse = cleanJsonResponse(response);
    
    logStep('JobDetails', 'Parsing response');
    const parsed = JSON.parse(cleanedResponse);
    
    logStep('JobDetails', 'Extraction completed successfully');
    return parsed;
  } catch (error) {
    logStep('JobDetails Error', {
      message: error.message,
      type: error.name
    });
    throw new Error(`Job details extraction failed: ${error.message}`);
  }
};
...
In your evaluation, first thoroughly analyze the job description to identify key requirements, then evaluate the candidate's qualifications against those specific requirements. Ensure both aspects receive equal attention in your assessment.

Return ONLY a raw JSON object that provides a detailed, realistic assessment.`
    },
    {
      role: 'user',
...
Step 3: Return a JSON object with the following structure:
...
Use a 0-100 scale for numerical ratings where:
- 0-20: Significantly under-qualified
- 21-40: Missing critical requirements
- 41-60: Meets some requirements but has significant gaps
- 61-80: Qualified with minor gaps
...
1. Use the exact name and personal details from the provided resume
2. Do not make up or change any personal information
3. Only optimize the content and formatting
4. Return the cover letter in plain text format`
    }
  ];
...
export const generateFollowUpEmail = async (resume, jobDescription, analysis, apiKey, template = 'MODERN') => {
  logStep('Email', 'Starting follow-up email generation');
  
  const messages = [
    {
      role: 'system',
      content: `You are an email writing expert. Generate a professional follow-up email based on the resume and analysis feedback. Focus on creating a concise, effective message.

Return plain text formatted email with appropriate paragraph breaks and structure.`
    },
    {
      role: 'user',
      content: `Generate a professional follow-up email based on the resume and analysis feedback.
...
2. Do not make up or change any personal information
3. Only optimize the content and formatting
4. Return the email in plain text format`
    }
  ];
